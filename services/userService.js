...
const deleteById = id =>
    userStorage.deleteById(id).then(affectedRows => ({ affectedRows })); //Kurzschreibweise!

const update = async (id, { username, firstName, lastName, password }) => {
    //check for duplication user name
    if (username) {
    ...
    return userStorage
        ...
        .then(affectedRows => ({ affectedRows })); //Kurzschreibweise
};
